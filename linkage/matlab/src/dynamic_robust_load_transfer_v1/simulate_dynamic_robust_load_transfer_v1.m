...
r3b_enabled=isfield(config,'r3b_recontact_enabled') && ...
...
recontact_target = struct('found',false,'q',q_start,'path_q',q_start, ...
    ...
    'control_q',q_start,'control_dq',zeros(2,1));
...
minimum_gap=nan(1,n_max);minimum_gap_velocity=nan(1,n_max);
contact_margin=nan(1,n_max);recontact_target_force=nan(1,n_max);
recontact_reference_offset=zeros(2,n_max);
recontact_stable=false(1,n_max);recontact_stable_time=zeros(1,n_max);
recontact_stage=strings(1,n_max);recontact_stage(:)="NOT_ACTIVE";
...
for index=1:n_max
    ...
    [minimum_gap_now,minimum_gap_index]=min(bed.gap_m);
    minimum_gap_velocity_now= ...
        bed.points.velocity_world(2,minimum_gap_index);
    contact_margin_now=bed.total_normal_force_N- ...
    ...
    if manager.mode=="SUSPENDED_MOTION" && return_phase && bed_absent && ...
        ...
        if candidate.found
            ...
            recontact_target.control_q=candidate.q;
            recontact_target.control_dq=zeros(2,1);
            reference=reference_for_mode(manager,progress,transition_q, ...
                transfer_target,recontact_target,plan,config);
            dynamic=dynamic_robust_v1_dynamic_margin(q,dq,reference.q, ...
                reference.dq,reference.ddq,controller_model,config);
    ...
    if r3b_enabled
        signals.recontact_stable=bed.total_normal_force_N>= ...
            config.r3b_contact_target_N-config.r3b_target_tolerance_N && ...
            contact_margin_now>=0 && ...
            abs(minimum_gap_velocity_now)<= ...
            config.r3b_gap_velocity_tolerance_m_s && ...
            max(bed.penetration_m)<=config.r3b_max_penetration_m && ...
            bed.total_normal_force_N<=config.r3b_max_bed_force_N;
        signals.recontact_unsafe=max(bed.penetration_m)> ...
            config.r3b_max_penetration_m || ...
            bed.total_normal_force_N>config.r3b_max_bed_force_N;
    else
        signals.recontact_stable=bed_supported;
        signals.recontact_unsafe=false;
    end
    ...
    if manager.mode~=old_mode
        ...
        if r3b_enabled && manager.mode=="RECONTACT"
            recontact_target.control_q=q;
            recontact_target.control_dq=zeros(2,1);
        end
    end
    if r3b_enabled && manager.mode=="RECONTACT"
        previous_command=recontact_target.control_q;
        [recontact_target.control_q,recontact_details]= ...
            r3b_recontact_margin_step(previous_command, ...
            recontact_target.path_q,recontact_target.tube_rad, ...
            bed.total_normal_force_N,controller_model,h_hip,config);
        recontact_target.control_dq=recontact_details.command_rate_rad_s;
    elseif r3b_enabled && manager.mode=="LOAD_RETURN"
        recontact_target.control_dq=zeros(2,1);
    end
    reference = reference_for_mode(manager, progress, transition_q, ...
        transfer_target, recontact_target, plan, config);
    ...
    minimum_gap(index)=minimum_gap_now;
    minimum_gap_velocity(index)=minimum_gap_velocity_now;
    contact_margin(index)=contact_margin_now;
    if r3b_enabled
        recontact_target_force(index)=config.r3b_contact_target_N;
    else
        recontact_target_force(index)=config.contact_force_threshold_N;
    end
    recontact_reference_offset(:,index)=reference.q-nominal_path.q;
    recontact_stable(index)=signals.recontact_stable;
    recontact_stable_time(index)=manager.recontact_stable_time_s;
    if manager.mode=="LOAD_RETURN"
        recontact_stage(index)="LOAD_RETURN";
    elseif manager.mode=="RECONTACT" && signals.recontact_stable
        recontact_stage(index)="STABLE_CONTACT";
    elseif manager.mode=="RECONTACT"
        recontact_stage(index)="CONTROLLED_CONTACT_BUILD";
    elseif return_phase && active_contacts(index)>0 && ...
            (index==1 || active_contacts(index-1)==0)
        recontact_stage(index)="FIRST_CONTACT";
    elseif return_phase && recontact_target.found
        recontact_stage(index)="APPROACH";
    end
...
fields={'t','state','q_ref','dq_ref','ddq_ref','q_nominal','tube_rad', ...
    ...
    'max_penetration_m','minimum_gap_m','minimum_gap_velocity_m_s', ...
    'contact_margin_N','recontact_target_force_N', ...
    'recontact_reference_offset_rad','recontact_stable', ...
    'recontact_stable_time_s','recontact_stage', ...
    'soft_limit_active','soft_limit_torque_Nm', ...
...
values={t,x,q_ref,dq_ref,ddq_ref,q_nominal,tube_rad,task_s,mode, ...
    ...
    minimum_gap,minimum_gap_velocity,contact_margin, ...
    recontact_target_force,recontact_reference_offset,recontact_stable, ...
    recontact_stable_time,recontact_stage,soft_limit_active, ...
    soft_limit_torque,rom_violation,inside_tube, ...
...
switch manager.mode
    ...
    case {"RECONTACT","LOAD_RETURN","BED_SUPPORTED_RETURN"}
        ...
        return_transition_q=transition_q;
        if isfield(config,'r3b_recontact_enabled') && ...
                config.r3b_recontact_enabled && ...
                isfield(recontact_target,'control_q')
            return_transition_q=recontact_target.control_q;
        end
        reference=dynamic_robust_v1_return_reference(manager.mode, ...
            return_transition_q,return_path_q,nominal,progress, ...
            manager.mode_time_s,config.load_return_duration_s);
        if manager.mode=="RECONTACT" && ...
                isfield(recontact_target,'control_dq')
            reference.dq=recontact_target.control_dq;
        end
    otherwise
