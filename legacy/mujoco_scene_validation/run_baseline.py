"""
Run baseline (ImpedanceMPC) on MuJoCo, record GIF via offscreen renderer.
Usage: python run_baseline.py [--offset 30] [--target 90]
"""
...
from rod_traction_baseline import ImpedanceMPC
...
def run(offset_deg, target_deg):
    ...
    ctrl = ImpedanceMPC(phi_offset_deg=offset_deg)
    ...
    print(f"[Baseline] offset={offset_deg}° target={target_deg}°")
    ...
    print(f"  Snaps            : {sum(ctrl.snap_log)}")
