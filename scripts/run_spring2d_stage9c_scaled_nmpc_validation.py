"""Stage 9C scaled alpha-slack NMPC closed-loop validation."""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "stage9c_scaled_nmpc_validation"
CONDITIONS_PHASE1 = ["clean"]
CONDITIONS_PHASE2 = ["noise", "noise_bias"]
SEEDS = [101, 102, 103, 104, 105]
SLACK_ACTIVE_THRESHOLD = 1.0e-5
...
EXTRA_FIELDS = [
    ...
    "nmpc_alpha_slack_active_count_thresholded",
    "nmpc_alpha_slack_active_threshold",
    "nmpc_first_F_tan",
    "nmpc_first_F_rad",
...
class ScaledValidationNMPC(DiagnosticCasadiNMPC):
    ...
    def _build_solver(self) -> Any:
        ...
        return ca.nlpsol(f"stage9c_{safe_name}_{ScaledValidationNMPC._counter}", "ipopt", nlp, opts)
    ...
    def _diagnostics(
        ...
        return {
            ...
            "alpha_slack_active_count": int(np.count_nonzero(slacks > SLACK_ACTIVE_THRESHOLD)),
    ...
    def act(self, observation: Any) -> np.ndarray:
        ...
        self.last_diag = {
            ...
            **cost_terms,
            ...
            "nmpc_alpha_slack_active_count_thresholded": int(diag["alpha_slack_active_count"]),
            "nmpc_alpha_slack_active_threshold": float(SLACK_ACTIVE_THRESHOLD),
...
def configure_nmpc_run(base_cfg: dict[str, Any], method: str, seed: int) -> dict[str, Any]:
    ...
    solver["horizon"] = 18
    cfg["mpc_params"]["stage9c_nmpc"] = method_variant(method)
    return cfg
...
def condition_with_seed(base_cfg: dict[str, Any], condition: str, seed: int) -> dict[str, Any]:
    condition_cfg = copy.deepcopy(base_cfg["conditions"][condition])
...
def _add_extra_fields(row: dict[str, Any], diag: dict[str, Any] | None = None) -> dict[str, Any]:
    ...
    for key in EXTRA_FIELDS:
        ...
        elif key in {"nmpc_solver_iterations", "nmpc_alpha_slack_active_count_thresholded", "nmpc_horizon"}:
...
def run_nmpc_condition(method: str, condition: str, seed: int, condition_cfg: dict[str, Any], cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def coupling_diagnostics(result: Any, parameter_step_norm: float) -> dict[str, Any]:
        return {
            ...
            "coupling_case": "stage9c_scaled_nmpc_validation",
...
def summarize_rows(method: str, condition: str, seed: int, phase: str, rows: list[dict[str, Any]], cfg: dict[str, Any], runtime_s: float) -> dict[str, Any]:
    ...
    return {
        ...
        "slack_active_count_thresholded": int(np.nansum(_series(decisions, "nmpc_alpha_slack_active_count_thresholded"))) if decisions else 0,
        "slack_active_threshold": SLACK_ACTIVE_THRESHOLD,
        ...
        out.append(
            {
                ...
                "slack_active_total": int(sum(int(row["slack_active_count_thresholded"]) for row in group)),
                "omega_p95_avg": _finite_mean(np.array([float(row["omega_p95_severity"]) for row in group])),
                "omega_max_avg": _finite_mean(np.array([float(row["omega_max_severity"]) for row in group])),
...
def save_plots(
    ...
    conditions = sorted({row["condition"] for row in agg})
    for metric, ylabel, filename in [
        ...
        ("slack_active_total", "slack active count", "slack_active_bar.png"),
    ...
    for condition in sorted({key[1] for key in all_runs}):
        for key, ylabel, filename in [
            ...
            if key == "alpha_step":
                ax.axhline(3.0, color="black", linestyle=":", linewidth=1.0, label="alpha threshold")
...
def save_report(summary_rows: list[dict[str, Any]], output_root: Path, commands: list[str], phase2_skipped_reason: str | None) -> None:
    ...
    clean_baseline = agg[("baseline_cem", "clean")]
    clean_nmpc = agg.get(("nmpc_rho100_scaled", "clean"))
    clean_fb = agg.get(("nmpc_rho100_scaled_with_cem_fallback", "clean"))
    clean_1000 = agg.get(("nmpc_rho1000_scaled", "clean"))
    clean_reliable = clean_nmpc is not None and float(clean_nmpc["solver_failure_rate_avg"]) < 0.05
    nmpc_preserves = clean_nmpc is not None and int(clean_nmpc["target_success"]) >= int(clean_baseline["target_success"])
    alpha_reduces = (
        clean_nmpc is not None
        and float(clean_nmpc["alpha_p95_avg"]) < float(clean_baseline["alpha_p95_avg"])
        and float(clean_nmpc["alpha_p99_avg"]) < float(clean_baseline["alpha_p99_avg"])
        and float(clean_nmpc["alpha_max_avg"]) < float(clean_baseline["alpha_max_avg"])
    )
    avoids_other = (
        clean_nmpc is not None
        and float(clean_nmpc["omega_p95_avg"]) <= float(clean_baseline["omega_p95_avg"])
        and int(clean_nmpc["delta_r_count_total"]) <= int(clean_baseline["delta_r_count_total"])
        and int(clean_nmpc["force_count_total"]) <= int(clean_baseline["force_count_total"])
    )
    rho1000_helps = (
        clean_1000 is not None
        and clean_nmpc is not None
        and float(clean_1000["alpha_max_avg"]) < float(clean_nmpc["alpha_max_avg"])
        and int(clean_1000["target_success"]) >= int(clean_nmpc["target_success"])
    )
    fallback_helps = (
        clean_fb is not None
        and clean_nmpc is not None
        and float(clean_fb["fallback_rate_avg"]) > 0.0
        and float(clean_fb["solver_failure_rate_avg"]) <= float(clean_nmpc["solver_failure_rate_avg"])
        and int(clean_fb["target_success"]) >= int(clean_nmpc["target_success"])
    )
    if phase2_skipped_reason:
        next_step = "NMPC refinement before stress validation."
    elif clean_reliable and nmpc_preserves and alpha_reduces and avoids_other:
        next_step = "stress validation."
    elif clean_reliable and not alpha_reduces:
        next_step = "alpha/task redesign or NMPC refinement; do not prepare linked rods yet."
    else:
    ...
    lines = [
        ...
        f"- Meaningful slack activity threshold: s_alpha > {SLACK_ACTIVE_THRESHOLD:g}.",
    ...
    for row in aggregate(summary_rows):
        lines.append(
            f"| {row['method']} | {row['condition']} | {row['target_success']}/{row['n']} | "
            ...
            f"{_fmt(row['alpha_duration_avg'])} | {_fmt(row['alpha_integral_avg'])} | {_fmt(row['slack_mean_avg'])} | "
            f"{_fmt(row['slack_max_avg'])} | {row['slack_active_total']} | {_fmt(row['omega_p95_avg'])} | {_fmt(row['omega_max_avg'])} | "
            f"{row['delta_r_count_total']} | {row['force_count_total']} |"
        )
    if phase2_skipped_reason:
        lines.extend(["", "## Stop Condition", f"- Phase 2 skipped: {phase2_skipped_reason}"])
    lines.extend(
        [
            "",
            "## Required Answers",
            "1. Does scaled NMPC solve reliably on clean multi-seed?",
            f"- {'Yes' if clean_reliable else 'No'}: clean rho100 solver failure rate={_fmt(clean_nmpc['solver_failure_rate_avg']) if clean_nmpc else 'nan'}.",
            "",
            "2. Does it preserve target reaching vs baseline CEM?",
            f"- {'Yes' if nmpc_preserves else 'No'}: clean rho100 target={clean_nmpc['target_success'] if clean_nmpc else 'nan'}/{clean_nmpc['n'] if clean_nmpc else 'nan'}, baseline={clean_baseline['target_success']}/{clean_baseline['n']}.",
            "",
            "3. Does it reduce alpha p95/p99/max/duration/integral?",
            f"- {'Yes' if alpha_reduces else 'No/mixed'}: clean rho100 alpha p95/p99/max/duration/integral={_fmt(clean_nmpc['alpha_p95_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['alpha_p99_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['alpha_max_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['alpha_duration_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['alpha_integral_avg']) if clean_nmpc else 'nan'}, baseline={_fmt(clean_baseline['alpha_p95_avg'])}/{_fmt(clean_baseline['alpha_p99_avg'])}/{_fmt(clean_baseline['alpha_max_avg'])}/{_fmt(clean_baseline['alpha_duration_avg'])}/{_fmt(clean_baseline['alpha_integral_avg'])}.",
            "",
            "4. Does it avoid worsening omega/delta_r/force violations?",
            f"- {'Yes' if avoids_other else 'No/mixed'}: clean rho100 omega p95/max={_fmt(clean_nmpc['omega_p95_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['omega_max_avg']) if clean_nmpc else 'nan'}, delta_r={clean_nmpc['delta_r_count_total'] if clean_nmpc else 'nan'}, force={clean_nmpc['force_count_total'] if clean_nmpc else 'nan'}.",
            "",
            "5. How often is alpha slack meaningfully active?",
            f"- Clean rho100 thresholded active count={clean_nmpc['slack_active_total'] if clean_nmpc else 'nan'} with threshold {SLACK_ACTIVE_THRESHOLD:g}; mean/max slack={_fmt(clean_nmpc['slack_mean_avg']) if clean_nmpc else 'nan'}/{_fmt(clean_nmpc['slack_max_avg']) if clean_nmpc else 'nan'}.",
            "",
            "6. Is rho100 enough, or does rho1000 help?",
            f"- {'rho1000 helps on clean alpha max' if rho1000_helps else 'rho1000 does not materially help clean aggregate alpha max/target'}: rho1000 alpha max={_fmt(clean_1000['alpha_max_avg']) if clean_1000 else 'nan'}, rho100 alpha max={_fmt(clean_nmpc['alpha_max_avg']) if clean_nmpc else 'nan'}.",
            "",
            ...
            f"- {'Yes/mixed' if fallback_helps else 'No material effect observed'}: fallback clean target={clean_fb['target_success'] if clean_fb else 'nan'}, fallback rate={_fmt(clean_fb['fallback_rate_avg']) if clean_fb else 'nan'}.",
            "",
    ...
    (output_root / "stage9c_report.md").write_text("\n".join(lines) + "\n")


def clean_phase_reasonable(summary_rows: list[dict[str, Any]]) -> tuple[bool, str | None]:
    agg = _agg_map(summary_rows)
    row = agg.get(("nmpc_rho100_scaled", "clean"))
    if row is None:
        return False, "missing clean nmpc_rho100_scaled result"
    if float(row["solver_failure_rate_avg"]) > 0.2:
        return False, f"clean solver failure rate was {row['solver_failure_rate_avg']:.3g}"
    if int(row["target_success"]) < 4:
        return False, f"clean target success was {row['target_success']}/{row['n']}"
    return True, None
...
def run(output_root: Path, config_path: Path) -> None:
    try:
        ...
        raise RuntimeError("CasADi unavailable for Stage 9C.") from exc
    print(f"[stage9c] CasADi {casadi_version}; seeds={SEEDS}", flush=True)
    ...
    def run_one(method: str, condition: str, seed: int, phase: str) -> None:
        print(f"[stage9c] running {phase} {method}/{condition}/seed{seed}", flush=True)
        condition_cfg = condition_with_seed(base_cfg, condition, seed)
        if method in {"baseline_cem", "alpha200_omega0"}:
            cfg = configure_cem_seeded(base_cfg, method, seed)
            start = time.perf_counter()
            rows = run_condition(condition, condition_cfg, cfg)
        else:
            cfg = configure_nmpc_run(base_cfg, method, seed)
            start = time.perf_counter()
    ...
    for seed in SEEDS:
        for method in METHODS:
            run_one(method, "clean", seed, "phase1_clean")
    ok, reason = clean_phase_reasonable(summary_rows)
    phase2_skipped_reason = None
    if ok:
        for condition in CONDITIONS_PHASE2:
            for seed in SEEDS:
                for method in METHODS:
                    run_one(method, condition, seed, "phase2")
    else:
        phase2_skipped_reason = reason
    save_summary(summary_rows, output_root / "stage9c_summary.csv")
    save_plots(summary_rows, output_root, all_runs)
    save_report(summary_rows, output_root, commands, phase2_skipped_reason)
    print(f"[stage9c] summary: {output_root / 'stage9c_summary.csv'}", flush=True)
    print(f"[stage9c] report : {output_root / 'stage9c_report.md'}", flush=True)
    print(f"[stage9c] figs   : {output_root / 'figs'}", flush=True)
