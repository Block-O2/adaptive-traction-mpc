...
def test_selected_levels_exist_in_frozen_source_configs() -> None:
    ...
    patient_path = resolve_stage_artifact(STAGE_ROOT, patient_source["path"])
    trajectory_path = resolve_stage_artifact(STAGE_ROOT, trajectory_source["path"])
    audit_path = resolve_stage_artifact(STAGE_ROOT, audit_source["path"])
...
def test_execution_counts_and_bridge_hashes_are_exact() -> None:
    ...
    for case in bridge_cases:
        source_dir = resolve_stage_artifact(STAGE_ROOT, case["source_result_directory"])
