...
`results/robustness/crossed_replication/summary/research_report.md`。**
...
完整 singular spectra、`I=X^T X`、information diagonal 与最弱三个 span-normalized directions 固定引用 `results/robustness/trajectory_excitation/design_audit.json`，不得用未来 realized outcomes 替换。主要弱方向为：
