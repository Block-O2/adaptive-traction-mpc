...
[`STAGE4_REPOSITORY_CLEANUP_AUDIT.md`](STAGE4_REPOSITORY_CLEANUP_AUDIT.md).

Stage 4 is now ready to freeze before a new report-validation branch. The next
phase is professor-facing baseline comparison and visualization: PD versus
PD+feedforward versus fixed-model MPC versus adaptive MPC, plus representative
patient/trajectory GIF or video experiments. That work requires a new approved
Experiment Spec and must not overwrite this evidence. Hardware preparation
should validate the CR12 command/feedback contract, timing, sensor calibration,
