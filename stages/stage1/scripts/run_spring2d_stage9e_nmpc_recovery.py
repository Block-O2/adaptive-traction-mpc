"""Stage 9E NMPC recovery/progress refinement for adverse initial states."""
...
DEFAULT_OUTPUT_ROOT = PROJECT_ROOT / "results" / "stage9e_nmpc_recovery"
PHASE1_CONDITIONS = ["initial_theta_offset"]
PHASE2_CONDITIONS = ["clean", "noise", "noise_bias", "stronger_noise"]
...
METHODS = [
    "baseline_cem",
    "nmpc_base",
    "nmpc_terminal_progress",
    "nmpc_progress_stage_cost",
    "nmpc_longer_horizon",
]
NMPC_METHODS = {
    "nmpc_base",
...
class ScaledValidationNMPC(DiagnosticCasadiNMPC):
    ...
    def _build_solver(self) -> Any:
        ...
        return ca.nlpsol(f"stage9e_{safe_name}_{ScaledValidationNMPC._counter}", "ipopt", nlp, opts)
...
def method_variant(method: str) -> dict[str, Any]:
    ...
    if method == "nmpc_terminal_progress":
        variant["terminal_theta_multiplier"] = 3.0
    elif method == "nmpc_progress_stage_cost":
        variant["progress_weight"] = 250.0
    elif method == "nmpc_longer_horizon":
        variant["horizon"] = 30
    return variant
...
def configure_nmpc_run(base_cfg: dict[str, Any], method: str, seed: int) -> dict[str, Any]:
    ...
    solver["horizon"] = 18
    cfg["mpc_params"]["stage9e_nmpc"] = method_variant(method)
    return cfg
...
def apply_stage9e_overrides(cfg: dict[str, Any], condition: str) -> dict[str, Any]:
...
def run_nmpc_condition(method: str, condition: str, seed: int, condition_cfg: dict[str, Any], cfg: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def coupling_diagnostics(result: Any, parameter_step_norm: float) -> dict[str, Any]:
        return {
            ...
            "coupling_case": "stage9e_nmpc_recovery",
...
def save_plots(
    ...
    colors = {
        ...
        "nmpc_base": "tab:green",
        "nmpc_terminal_progress": "tab:orange",
        "nmpc_progress_stage_cost": "tab:purple",
        "nmpc_longer_horizon": "tab:red",
    }
...
def save_report(summary_rows: list[dict[str, Any]], output_root: Path, commands: list[str], phase2_skipped_reason: str | None) -> None:
    ...
    base_near_miss = (
        base is not None
        and int(base["target_success"]) == 0
        and float(base["theta_error_final_avg"]) < 0.1
        and float(base["theta_error_reduction_avg"]) > 80.0
    )
    base_wrong_direction = base is not None and float(base["theta_error_reduction_avg"]) < 0.0
    base_stalls = (
        base is not None
        and int(base["target_success"]) == 0
        and not base_near_miss
        and not base_wrong_direction
        and float(base["theta_error_reduction_avg"]) < 30.0
    )
    ...
    if winner and alpha_gain_preserved and not phase2_regression:
        next_step = "NMPC refinement/stress rerun with the recovery variant before linked-rods preparation."
    elif winner:
        next_step = "NMPC refinement; recovery improved target success but introduced regression risk."
    else:
        next_step = "dedicated recovery mode or task/progress redesign before linked-rods preparation."
    lines = [
    ...
    for row_data in aggregate_rows:
        lines.append(
            ...
            f"{_fmt(row_data['solver_failure_rate_avg'])} | {_fmt(row_data['solve_time_avg'])} | "
            f"{_fmt(row_data['theta_error_final_avg'])} | {_fmt(row_data['theta_error_reduction_avg'])} | "
            f"{_fmt(row_data['pred_progress_avg'])} | {_fmt(row_data['pred_final_error_avg'])} | "
            f"{_fmt(row_data['alpha_p95_avg'])} | {_fmt(row_data['alpha_max_avg'])} | {_fmt(row_data['omega_abs_max_avg'])} | "
            f"{_fmt(row_data['slack_max_avg'])} | "
    ...
    lines.extend(
        [
            ...
            f"- {'Near-target underreach, not gross stalling' if base_near_miss else 'Wrong-direction motion' if base_wrong_direction else 'Low-motion stall' if base_stalls else 'Mixed/unclear'}: base target={base['target_success'] if base else 'nan'}/{base['n'] if base else 'nan'}, final error={_fmt(base['theta_error_final_avg']) if base else 'nan'} deg, theta-error reduction={_fmt(base['theta_error_reduction_avg']) if base else 'nan'} deg, predicted progress={_fmt(base['pred_progress_avg']) if base else 'nan'} deg. The environment requires theta >= theta_target; base ends just below the crossing threshold.",
            "",
            "2. Is the cause weak progress incentive, short horizon, or alpha conservatism?",
            f"- Evidence points to {'the winning recovery variant below' if winner else 'near-target crossing/settling logic plus alpha/action conservatism, not a simple horizon-only fix'}: winner={winner or 'none'}. Terminal-progress and longer-horizon variants did not restore target crossing; progress-stage-cost caused high solver failure. The initial reverse omega does not send the controller in the wrong direction, but the conservative solution avoids the final crossing margin.",
            "",
            "3. Which minimal variant restores recovery?",
            f"- {winner or 'No mandatory variant restored recovery to >=2/3 target success'}.",
            "",
            "4. Does recovery improvement preserve alpha-tail gains?",
            f"- {'Yes' if alpha_gain_preserved else 'No/mixed'}: initial_theta_offset winner alpha max={_fmt(row(winner, 'initial_theta_offset')['alpha_max_avg']) if winner else 'nan'}, baseline CEM alpha max={_fmt(baseline['alpha_max_avg']) if baseline else 'nan'}.",
            "",
            "5. Is a dedicated recovery mode necessary?",
            f"- {'Not necessary for this failure if the winning variant is kept, but should still be reserved for future adverse-state stress.' if winner else 'Yes, likely necessary: the mandatory variants did not recover target reaching.'}",
            "",
            "6. Is the controller ready for linked-rods preparation after this?",
            f"- {'No, not yet' if (not winner or phase2_regression) else 'Not directly; run a Stage 9D-style stress rerun with the chosen recovery variant first'}: {next_step}",
            f"- Recommended next step: {next_step}",
        ]
    )
    (output_root / "stage9e_report.md").write_text("\n".join(lines) + "\n")
...
def recovery_methods_for_phase2(summary_rows: list[dict[str, Any]]) -> tuple[list[str], str | None]:
    ...
    if not winners:
        return [], "no recovery variant reached >=2/3 target success on initial_theta_offset"
    return winners, None
...
def run(output_root: Path, config_path: Path) -> None:
    try:
        ...
        raise RuntimeError("CasADi unavailable for Stage 9E.") from exc
    print(f"[stage9e] CasADi {casadi_version}; seeds={SEEDS}", flush=True)
    ...
    def run_one(method: str, condition: str, seed: int, phase: str) -> None:
        print(f"[stage9e] running {phase} {method}/{condition}/seed{seed}", flush=True)
        condition_cfg = condition_with_seed(base_cfg, condition, seed)
        if method == "baseline_cem":
            cfg = apply_stage9e_overrides(configure_cem_seeded(base_cfg, method, seed), condition)
            start = time.perf_counter()
            rows = run_condition(condition, condition_cfg, cfg)
        else:
            cfg = apply_stage9e_overrides(configure_nmpc_run(base_cfg, method, seed), condition)
            start = time.perf_counter()
    ...
    for seed in SEEDS:
        for method in METHODS:
            run_one(method, "initial_theta_offset", seed, "phase1_recovery")
    ...
    save_summary(summary_rows, output_root / "stage9e_summary.csv")
    save_plots(summary_rows, output_root, all_runs)
    save_report(summary_rows, output_root, commands, phase2_skipped_reason)
    print(f"[stage9e] summary: {output_root / 'stage9e_summary.csv'}", flush=True)
    print(f"[stage9e] report : {output_root / 'stage9e_report.md'}", flush=True)
    print(f"[stage9e] figs   : {output_root / 'figs'}", flush=True)
